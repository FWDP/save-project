...
export default function AddExpenseScreen() {
  ...
  const addField = () =>
    patchDraft({
      customFields: [...draft.customFields, { label: '', value: '' }],
    });
  const updateField = (index: number, key: 'label' | 'value', value: string) =>
    patchDraft({
      customFields: draft.customFields.map((field, fieldIndex) =>
  ...
  const submit = async () => {
    ...
    if (
      ...
      setMessage('Date, amount, category, and description are required.');
      return;
    }
    setSaving(true);
    try {
      ...
      const created = await saveTransactionDraft({
        userId: '',
        type: draft.type,
        amount,
        ...
        tags: draft.tags
          ...
          .map((tag) => tag.trim())
          .filter(Boolean),
        recurring: draft.type === 'expense' && draft.recurring,
    ...
    >
      <View style={styles.segment}>
        ...
        <Pressable
          style={[
            styles.segmentButton,
            draft.type === 'income' && styles.incomeActive,
          ]}
      ...
      <View style={styles.details}>
        ...
        {draft.customFields.length ? (
          draft.customFields.map((field, index) => (
            <View key={index} style={styles.twoColumns}>
              <Field
                label="Field name"
                value={field.label}
          ...
          <Text style={styles.help}>
            No extra fields yet. Add fields like “Project”, “Client”, or “Tax
            Rate”.
          </Text>
        )}
        <Pressable style={styles.dashedButton} onPress={addField}>
          <Text style={styles.dashedText}>＋ Add Custom Field</Text>
        </Pressable>
      </View>
      <Field
        label="Tags"
        value={draft.tags}
        onChangeText={(tags) => patchDraft({ tags })}
        placeholder="Add comma-separated tags…"
        help="Commas separate tags"
      />
      {draft.type === 'expense' ? (
        <View style={styles.switchRow}>
          ...
          <Switch
            ...
            onValueChange={(recurring) => patchDraft({ recurring })}
            trackColor={{ false: '#21304a', true: '#5ca9ff' }}
          />
        </View>
      ) : null}
      <View>
      ...
      <View style={styles.actions}>
        ...
        <Pressable disabled={saving} style={styles.submit} onPress={submit}>
          <Text style={styles.submitText}>
            {saving
              ? 'Saving…'
              : `Add ${draft.type === 'expense' ? 'Expense' : 'Income'}`}
          </Text>
        </Pressable>
      </View>
    </FinancePage>
...
const styles = StyleSheet.create({
  ...
  expenseActive: { backgroundColor: '#d9474d' },
  incomeActive: { backgroundColor: '#18b95f' },
  segmentText: { color: '#f4f7fb', fontWeight: '800' },
  twoColumns: { flexDirection: 'row', gap: 12 },
  field: { flex: 1 },
  label: { color: '#e5ebf5', fontSize: 13, fontWeight: '700', marginBottom: 7 },
  input: {
  ...
  placeholder: { color: '#65738c' },
  inputText: { color: '#edf2fa', fontSize: 13 },
  ...
  newButtonText: { color: '#65acfa', fontWeight: '700' },
  options: {
  ...
  option: { flexDirection: 'row', alignItems: 'center', padding: 10, gap: 9 },
  dot: { width: 9, height: 9, borderRadius: 5 },
  optionText: { color: '#e7edf7' },
  details: {
    borderWidth: 1,
    borderColor: '#26334a',
    borderRadius: 10,
  ...
  legend: { color: '#9aa6ba', fontSize: 13, fontWeight: '700' },
  help: { color: '#8793a8', fontSize: 11, lineHeight: 17, marginTop: 6 },
  dashedButton: {
    borderWidth: 1,
    borderStyle: 'dashed',
    borderColor: '#263a58',
  ...
  dashedText: { color: '#9aa8bd', fontWeight: '700', fontSize: 12 },
  switchRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  switchText: { color: '#edf2fa', fontWeight: '700' },
  muted: { color: '#8b97aa', fontWeight: '400' },
  ...
  receiptIcon: { color: '#62aaff', fontSize: 27 },
  receiptTitle: { color: '#75b6ff', fontWeight: '700', marginTop: 8 },
  receiptMeta: { color: '#8793a8', fontSize: 11, marginTop: 7 },
  attachment: { color: '#21c985', fontSize: 10, marginTop: 8 },
  error: { color: '#ff788c', fontSize: 11 },
  actions: {
    flexDirection: 'row',
    justifyContent: 'flex-end',
    gap: 10,
    borderTopWidth: 1,
    borderTopColor: '#19263d',
    paddingTop: 12,
  },
  cancel: {
    borderWidth: 1,
  ...
  cancelText: { color: '#e6edf8', fontWeight: '700' },
  submit: {
